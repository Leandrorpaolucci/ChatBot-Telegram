...
def send_welcome(message):
	leh = bot.reply_to(message,"Este bot, foi criado em Python por Leandro Ribeiro Paolucci :)\nBem-vindo!!")
...
def process_age_step(message):
	try:
		...
		markup.add('São Paulo','Corinthians', 'Palmeiras') #quais as categorias
...
def process_team_step(message):
	try:
		...
		team = message.text
		user = user_dict[chat_id]
		if (team == u'São Paulo') or (team == u'Corinthians') or (team == u'Palmeiras'):
			user.team = team
		else:
			raise Exception()
